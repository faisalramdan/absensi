...
use App\Http\Controllers\LeaveRequestController; // SUDAH DIPERBAIKI (Huruf 'L' Besar)
...
Route::get('/api/employee/schedule-events', [DashboardController::class, 'getScheduleEvents'])
...
Route::middleware('auth')->group(function () {
    ...
    Route::resource('assignments', ShiftAssignmentController::class)->only(['index', 'create', 'store', 'destroy']);
    ...
    Route::prefix('teams/{team}')->group(function () {
        ...
        Route::get('/members/create', [TeamMemberController::class, 'create'])->name('teams.members.create');
        Route::post('/members', [TeamMemberController::class, 'store'])->name('teams.members.store');
        Route::get('/members/{member}/edit', [TeamMemberController::class, 'edit'])->name('teams.members.edit');
        Route::put('/members/{member}', [TeamMemberController::class, 'update'])->name('teams.members.update');
        Route::delete('/members/{member}', [TeamMemberController::class, 'destroy'])->name('teams.members.destroy');
    });

    Route::get('organization', [OrganizationStructureController::class, 'index'])->name('organization.index');

    // --- ATTENDANCE ---
    Route::get('attendance-logs/import', [AttendanceLogController::class, 'importForm'])->name('attendance-logs.import.form');
    Route::post('attendance-logs/import', [AttendanceLogController::class, 'import'])->name('attendance-logs.import');
    Route::resource('attendance-logs', AttendanceLogController::class)->except(['show']);
    Route::resource('attendances', AttendanceController::class)->only(['index', 'show']);

    Route::middleware(['permission:attendance-processor.view'])->group(function () {
        Route::get('attendance-processor', [AttendanceProcessorController::class, 'index'])->name('attendance-processor.index');
        Route::post('attendance-processor/generate', [AttendanceProcessorController::class, 'generate'])->name('attendance-processor.generate');
    });

    Route::get('attendance-monthly/employee/{employee}', [AttendanceMonthlyController::class, 'show'])->name('attendance-monthly.show');
    Route::resource('attendance-monthly', AttendanceMonthlyController::class)->only(['index']);

    // --- CONTRACTS & ALLOCATIONS ---
    Route::resource('employee-contracts', EmployeeContractController::class);
    Route::prefix('employee-contracts/{employeeContract}')->name('employee-contracts.')->group(function () {
        Route::get('/leave-allocations', [LeaveAllocationController::class, 'index'])->name('leave-allocations.index');
        Route::get('/leave-allocations/create', [LeaveAllocationController::class, 'create'])->name('leave-allocations.create');
        Route::post('/leave-allocations', [LeaveAllocationController::class, 'store'])->name('leave-allocations.store');
        Route::get('/leave-allocations/{leaveAllocation}', [LeaveAllocationController::class, 'show'])->name('leave-allocations.show');
        Route::get('/leave-allocations/{leaveAllocation}/edit', [LeaveAllocationController::class, 'edit'])->name('leave-allocations.edit');
        Route::put('/leave-allocations/{leaveAllocation}', [LeaveAllocationController::class, 'update'])->name('leave-allocations.update');
    });

    Route::resource('leave-allocations', LeaveAllocationController::class);
});

// ROUTE FIX AKSES - SAYA COMMENT UNTUK KEAMANAN PRODUCTION. 
// Hapus tanda // jika ingin menggunakannya kembali di lokal.
...
Route::get('/fix-akses', function () {
    try {
        ...
        return 'Berhasil! Cache bersih & Semua hak akses telah diberikan ke Admin.';
    } catch (\Exception $e) {
